...
public class SlingshotAmmo : MonoBehaviour
{
  ...
  void updateAudioPan() => audio.panStereo = (transform.position.x / (Camera.main.orthographicSize * 4f / 3f)) * .8f;
  ...
  void tiltSlingShot() => setSlingshotAngle((initialPosition.x - transform.position.x) * .01f);

  void setSlingshotAngle(float angle) => slingshotBase.transform.rotation = Quaternion.Euler(0f, 0f, angle * Mathf.Rad2Deg);
